...
import { PG_CONTAINER, containerNames, containerRunning } from '../containers'
...
export async function runReset(ctx: Ctx, mode: ResetMode): Promise<number> {
  ...
  const dbUrl = readEnvFile(ctx, 'ui/.env').DATABASE_URL
  ...
  const pgc = ctx.env.TALARIA_PG_CONTAINER ?? PG_CONTAINER
  if (!(await containerRunning(ctx, pgc))) {
    ctx.log.die(`postgres container '${pgc}' is not running — start it with \`bun talaria dev\``)
  }
...
create database talaria owner talaria;`,
  ...
  // fleet
  const agentCount = (await containerNames(ctx, 'talaria-fleet-agent')).length
  ctx.log.say('What this will remove')
  ctx.log.raw(row('running agent containers', agentCount))
  ...
  // A docker that refuses to list is not a reason to keep the rendered files:
  // containerNames answers [] instead of throwing.
  for (const n of await containerNames(ctx, 'talaria-fleet-agent', true)) {
    await ctx.exec('docker', ['rm', '-f', n]).catch(() => {})
  }

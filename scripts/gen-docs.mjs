...
// The Rust router-table parser lives in route-table.mjs now (shared with
// check-mcp-routes.mjs, so the docs and the mcp cross-check read the same
// truth). The functions moved verbatim; identical output is the invariant.
import { matchDelimRust, stripRustComments, parseRouterTable, ROUTE_GROUP_CRATES, moduleFile } from './route-table.mjs'
...
const GROUPS = {
  ...
  activity: { segs: ['activity', 'history', 'notifications', 'unreads', 'push', 'alerts', 'home', 'cost', 'host'], blurb: 'What happened, what it cost, what the machine is doing, what needs you.' },
...
/** Strip comments from TS text, LENGTH-PRESERVING — every
...
const table = parseRouterTable(modText, warnings)

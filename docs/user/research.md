...
| Do this | How |
| :--- | :--- |
| Run research | Type your question ("What should we find out?") → pick a **Depth** → **Start (⏎)** |
| Pick who researches | The agent picker at top — it also filters your history. "All agents" for anyone |
| Watch it run | The status panel shows each phase live — planning angles, searching, writing the report |
| Read the report | It appears beside the discussion, with a **N sources (M cited)** line and **Open in Artifacts** |
| Discuss the findings | **Start the discussion** — chat with the agent about its own report; it answers from what it found and can go look again |
...
sources that were consulted but not cited say so. If a run finds nothing citable, it stops
and asks you: **Search again** or **Stop**. Research without live search answers from
memory — and the citations come out invented — so an admin needs to have search configured
(you'll see the warning in Models if it isn't).
...
run is the owner's or an admin's. You can only research with agents you have access to.

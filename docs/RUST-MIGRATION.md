...
- **The org-voice blurb sweep does not run from Rust.** `maybeRewriteBlurbs`
  (the `rewritePendingBlurbs` harness run behind it) is scheduler plane —
  rule 5, batch 4 — so the Rust `/api/models` reads `model_blurbs` rows but
  never writes them. During coexistence the TS scheduler keeps the sweep, so
  blurbs stay fresh either way; after cutover the sweep ports with the runs
  plane. Until then the degradation is cosmetic and self-healing (the rows
  are read here the moment anyone writes them).

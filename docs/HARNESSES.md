...
| `server/harness/defs/*.ts` | The definitions themselves — 31 harnesses, 284 eval fixtures. |
...
- **Omitting the block entirely runs every enabled rule**, which is the opposite failure and just as
  ...
  model the admin had chosen to judge with. All 31 harnesses name their rules. A harness that
...
it. Twelve of the 31 harnesses widen; the titler deliberately does not, because a wider prompt would

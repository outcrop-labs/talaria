...
| [`stack/fleet.json`](./stack/fleet.example.json) | **fleet manifest** | Routing for the gateway plane (agent model → gateway url), regenerated by the renderer. Gitignored — keys live in `fleet/.env`. |
...
The gateway plane (the bridge that multiplexes agent chat — only needed once you
run agents):

```bash
cp stack/.env.example stack/.env
docker compose -f stack/docker-compose.yml up -d talaria-bridge
```

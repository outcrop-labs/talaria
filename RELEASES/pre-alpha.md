...
- **Develop with agents.** The Workbench: connect GitHub once, grant repos per agent, and agents build through a governed job lifecycle — Talaria cuts the branch, the plan posts to the ticket, coding harnesses (opencode, Pi, Oh My Pi) do the implementation with commits authored as the agent, and the ticket-linked PR comes back for human review.
...
- **Extend it.** Talaria is an app platform: self-contained codebases that compile into the deployment as native surfaces — their own APIs, per-app document stores, and MCP tools for your agents under the same governance. Install from the marketplace or any git URL; build with `@talaria/sdk` (`bun talaria app new`).

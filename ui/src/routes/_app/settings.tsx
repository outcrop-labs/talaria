...
function PreferredModelPicker() {
  ...
  const failed = catalogQuery.isError ? catalogQuery : prefsQuery.isError ? prefsQuery : null
  const { saved, flash } = useSavedFlash()
...
function NotificationsSection() {
  ...
  const [pending, setPending] = useState<Partial<NotifyPrefs>>({})
  const [pendingDigest, setPendingDigest] = useState<DigestPref | null>(null)
  const [pendingDelivery, setPendingDelivery] = useState<boolean | null>(null)
  const [error, setError] = useState<string | null>(null)
  const { saved, flash } = useSavedFlash()
  const prefs = data ? { ...data.prefs, ...pending } : null
  const digest: DigestPref | null = data ? (pendingDigest ?? data.digest) : null
  const emailOn = data ? (pendingDelivery ?? data.delivery.emailEnabled) : null

  const set = async (id: NotifyClass, route: NotifyRoute) => {
    setError(null)
    setPending((p) => ({ ...p, [id]: route }))
  ...
  return (
    <Panel as="section">
      ...
      {isPending ? (
        <SkeletonRows rows={6} />
      ) : !data || !prefs || !digest || emailOn === null ? (
            ...
            <div className="flex items-start gap-4">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span
                    aria-hidden
                    className={cn('h-1.5 w-1.5 shrink-0 rounded-full', emailOn ? 'bg-success' : 'bg-warning')}
                  />
                ...
                {data.canSetDelivery ? (
                  ...
                  <p className="mt-1 font-sans text-xs text-muted">Only an admin can turn it back on.</p>
                )}
              </div>
              {data.canSetDelivery && (
                <Segmented
        ...
        <ul className="divide-y divide-line-subtle">
          {NOTIFY_CLASSES.map((c) => (
            <li key={c.id} className="flex items-center gap-4 py-3">
              <div className="min-w-0 flex-1">
                ...
                <div className="font-sans text-xs text-muted">{c.blurb}</div>
              </div>
              <Segmented
          ...
          <li className="flex items-center gap-4 py-3">
            <div className="min-w-0 flex-1">
              <div className="text-sm text-fg">Daily digest</div>

    ...
    {#if !searching && !folderId}
      <ul class="space-y-1" use:listStagger>
        {#each childFolders as f (f.id)}
          <li
            class="flex items-center gap-2 rounded-md border border-line px-3 py-2 transition-colors hover:dither-fill"
          ...
          <li class="rounded-md border border-line p-3" transition:slide oncontextmenu={(e) => rowMenu(e, s)}>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-1.5">
              ...
              <button
                ...
                class="ml-auto grid h-7 w-7 shrink-0 place-items-center rounded-md text-muted transition-colors hover:dither-fill hover:text-fg"
              >
                <MoreHorizontal size={14} aria-hidden="true" />
              </button>
              </div>

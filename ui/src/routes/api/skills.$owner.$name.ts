...
// skill if new). DELETE → remove the whole skill dir. Writes are admin-only;
// edits are LIVE (Hermes reads skills per invocation, no restart).
  ...
  server: {
    handlers: {
      ...
      PUT: async ({ request, params }) => {
        ...
        try {
          await writeSkill(params.owner, params.name, parsed.data.content)
          return json({ ok: true })
        } catch (e) {

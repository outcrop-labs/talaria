...
export const Route = defineApi('/api/boards/$id', {
  PATCH: async ({ request, params }) => {
    ...
    if (teamId === undefined && body.teamName !== undefined) {
      const name = (body.teamName ?? '').trim().toLowerCase()
      if (!name || name === 'personal') teamId = null
        ...
        if (!rows[0]) return json({ error: `no team named "${body.teamName}"` }, { status: 400 })
        teamId = rows[0].id
      }
    }
    if (teamId !== undefined) {
    ...
    if (body.name !== undefined) await renameBoard(params.id, body.name)
    if (body.archived !== undefined) await archiveBoard(params.id, body.archived)
    if (body.judgeMode !== undefined) await setBoardJudgeMode(params.id, body.judgeMode)
    return json({ ok: true })
  },

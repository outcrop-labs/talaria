<script lang="ts">
  ...
  let loadState = $state<{ space?: PublicSpace; missing?: boolean; error?: unknown }>({})
  let reload = $state(0)
  $effect(() => {
    void reload
    let live = true
    ...
    getJson<{ space: PublicSpace }>(`/api/kb/public/space/${slug}`)
      .then((d) => {
        if (live) loadState = { space: d.space }
      })
      .catch((e: unknown) => {
        if (!live) return
...
{:else}
  <PublicShell>
    <h1 class="mb-5 flex items-center gap-2 font-sans text-3xl font-semibold tracking-tight text-fg">
      <span>{loadState.space.icon ?? '📚'}</span>
      {loadState.space.name}
    </h1>
    {#if loadState.space.body.trim()}
      <Markdown class="tiptap" children={loadState.space.body} />
    {:else}

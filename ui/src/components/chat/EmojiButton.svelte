<script lang="ts">
  ...
  // The composer's smiley tile — a thin trigger over the ONE emoji picker
...
<EmojiPicker onPick={onPick} up>
  {#snippet trigger()}
    <IconButton title="Add emoji (or type :shortcode:)" size="tile" {disabled}>
      <Smile size={15} />
    </IconButton>
  {/snippet}
</EmojiPicker>

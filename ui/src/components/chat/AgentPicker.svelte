<script lang="ts">
  ...
  import { focusGold, popRow, popRowSelected } from '@/components/chat/chat-chrome'
  ...
  // The panel rides the §7 shell (ui/Popover): portaled out of the rail's
  // overflow, Esc/outside-click dismiss, glued to the trigger on scroll.
  let {
    agents,
    value,
    onChange,
...
<Popover bind:open follow offset={8} class="w-64">
  {#snippet trigger()}

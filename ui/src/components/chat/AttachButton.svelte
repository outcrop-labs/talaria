<script lang="ts">
  ...
  // the model and persists the chip on the message. The §7 shell (ui/Popover)
  // owns the portal/outside-click/Esc mechanics.
  ...
  // A closed menu restarts at its root pane, whatever closed it.
  $effect(() => {
    if (!open) pick = null
  })
...
<Popover bind:open follow up offset={6} class="w-64">
  {#snippet trigger()}
    ...
    <IconButton
      ...
      class="font-mono text-base leading-none"
    >
      {#if busy}

  ...
  <RailTooltip label={unseen ? `${label} · new output` : label}>
    {#if status === 'none'}
      <a
        ...
        class="relative grid h-9 w-9 place-items-center rounded-md text-muted transition-colors duration-[120ms] dither-fill hover:text-fg"
      >
    ...
    {:else}
      <button
        ...
        class={cn(
          'relative grid h-9 w-9 place-items-center rounded-md text-muted transition-colors duration-[120ms] dither-fill hover:text-fg',
...
{:else}
  <section aria-label="Your assistant" class="mt-3 shrink-0">
    ...
    {:else if status === 'none'}
      ...
      <a
        href="/settings/assistant"
        class="flex h-11 items-center gap-2.5 rounded-lg border border-dashed border-line px-2.5 text-left transition-colors duration-[120ms] hover:border-line-strong dither-fill"
      >

...
<div class="flex flex-col overflow-hidden rounded-md border border-line bg-card">
  <div class="flex shrink-0 flex-wrap items-center gap-x-3 gap-y-1 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.05em]">
    <button
      type="button"
      onclick={() => (open = !open)}
      class="-ml-1 flex items-center gap-1.5 rounded px-1 py-0.5 text-muted transition-colors hover:dither-fill hover:text-fg"
      aria-expanded={open}
    >
    ...
    {#if open}
      <span class="ml-auto">
        <button
          ...
          class={cn('rounded px-1.5 py-0.5 transition-colors', follow ? 'text-ink-dim' : 'text-accent hover:dither-fill')}
        >
      ...
      {#each live?.current ?? [] as c (keyOf(c))}
        ...
        <div>
          ...
          <button
            ...
            class="flex w-full flex-wrap items-baseline gap-x-2 gap-y-1 rounded px-1 py-0.5 text-left font-mono text-[11px] transition-colors enabled:hover:dither-fill"
          >

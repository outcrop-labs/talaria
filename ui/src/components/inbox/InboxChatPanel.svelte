...
{#if !collapsed}
  ...
  >
    ...
    <header class="flex h-12 shrink-0 items-center gap-3 border-b border-line px-4">
      ...
      <button type="button" onclick={collapse} aria-label="Collapse assistant conversation" aria-expanded={true} class="grid h-8 w-8 place-items-center rounded-md text-muted hover:dither-fill hover:text-fg">
        <ChevronLeft size={14} />
      </button>
    </header>
    ...
    <div class="shrink-0 border-t border-line bg-sidebar/95 p-2 backdrop-blur">
      ...
      {#if active}
        ...
        <div transition:slide|global={GROW_Y} class="mb-2 flex min-w-0 items-center gap-2 rounded-md border border-line bg-surface px-2.5 py-2">
          ...
          <button type="button" onclick={toggleActiveDecisionAttachment} aria-label={attached ? 'Detach active decision' : 'Attach active decision'} class="grid h-6 w-6 place-items-center rounded text-ink-dim hover:dither-fill hover:text-fg">

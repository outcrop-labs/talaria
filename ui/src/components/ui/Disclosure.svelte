...
<div class={cn('overflow-hidden rounded-md border border-line bg-card', className)}>
  <button
    type="button"
    onclick={() => (open = !open)}
    class="group/disc flex w-full cursor-pointer items-center gap-2 px-3 py-1.5 text-left font-mono text-[11px] tracking-[0.05em] text-muted transition-colors hover:dither-fill hover:text-fg"
  >

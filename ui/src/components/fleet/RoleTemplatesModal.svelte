<script lang="ts">
  ...
  // The role library, managed from AGENTS rather than Admin — it is part of
  // making an agent, not part of configuring the instance, and the person who
  // creates agents is the person who knows what the roles should be.
  //
  // Two-pane rather than one long column: a role carries a whole soul document,
  // and stacking that under six other fields is the shape that makes a dialog
  // scroll forever (see the note in Modal.svelte). List picks, pane edits.
...
<Modal {open} {onClose} title="Agent role templates" takeover padded={false}>

...
export function useInboxFocus(options: { enabled?: boolean } = {}) {
  return useQuery({
    queryKey: ['inbox-focus'],
    queryFn: ({ signal }) => getJson<FocusQueue>('/api/inbox/focus', signal),
...
export function useInboxFocusSummary(options: { enabled?: boolean } = {}) {
  return useQuery({
    queryKey: ['inbox-focus-summary'],
    queryFn: ({ signal }) => getJson<{ count: number }>('/api/inbox/focus/summary', signal),
    enabled: options.enabled,
    refetchInterval: 30_000,
    staleTime: 10_000,
  })
...
export function useInboxFocusConversation(options: { enabled?: boolean } = {}) {
  return useInfiniteQuery({
    queryKey: ['inbox-focus-conversation'],
    queryFn: ({ pageParam, signal }) => getJson<InboxConversationPage>(

...
| [`api/`](./api) | **the Rust api** — the backend (axum + sqlx): 216 of 219 route files serve from it in any proxied environment, and the scheduler runs from it when `TALARIA_SCHEDULER=rust`. The TS API stays for unproxied installs until cutover; the record and rules: [`docs/RUST-MIGRATION.md`](./docs/RUST-MIGRATION.md) | `bun run api` (dev, :5274); `bun run api:check` |

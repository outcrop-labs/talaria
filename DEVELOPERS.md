...
| [`mcp/`](./mcp) | **talaria-mcp** — the agent-facing MCP server: 63 tools, every call proxied to the app with the agent's own key; guardrails hold server-side (no assignee writes, no terminal status moves) | `cd mcp && bun i && bun run build` |

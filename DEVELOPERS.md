...
| [`api/`](./api) | **the Rust api** — the backend (axum + sqlx): every `/api/*` route except the four permanent TS residents serves from it (the TS API behind the proxy was deleted with the cutover), and the scheduler arms from it on boot unless `TALARIA_SCHEDULER=off`. Source is grouped one dir per subsystem — `api/src/routes/<group>/` mirrors the `docs/api` groups, engines sit in family dirs (`fleet/`, `google/`, `mcp/`, `model/`, `runs/`, `fitness/`, …). The record and rules: [`docs/RUST-MIGRATION.md`](./docs/RUST-MIGRATION.md) | `bun run api` (dev, :5274); `bun run api:check` |
| [`apps/`](./apps) | **Talaria apps** — self-contained TypeScript apps this instance compiles (marketplace or git). Core ships none; scaffold with `bun talaria app new` | see [`apps/README.md`](./apps/README.md) |
...
| [`mcp/`](./mcp) | **talaria-mcp** — the agent-facing MCP server: 61 tools, every call proxied to the app with the agent's own key; guardrails hold server-side (no assignee writes, no terminal status moves) | `cd mcp && bun i && bun run build` |
...
| [`docker/sidecars.compose.yml`](./docker) | **the shared sidecar plane** — Postgres, Redis, Qdrant, TEI embeddings, MinIO, SearXNG, images and healthchecks defined once and layered IN FRONT of every stack's own file (`docker/compose.yml`, `docker/dev-compose.yml`, `docker/devbox.compose.yml`) | `docker compose -f docker/sidecars.compose.yml config` |
| [`docker/dev-compose.yml`](./docker) | **dev infra** — the app runs on the host; the fleet runs under its own compose project. The sidecars themselves are the shared plane above | `docker compose -f docker/sidecars.compose.yml -f docker/dev-compose.yml config` |
...
| [`desktop/`](./desktop) | **Talaria Desktop** — the Tauri v2 multitenant shell: one window, a sidebar, and one isolated webview per instance's own web UI; its installers for linux, macOS and windows attach to every release. The story: [`docs/DESKTOP.md`](./docs/DESKTOP.md) | `bun run desktop` (dev; gates in the box, GUI on the host); `bun run desktop:check` |

...
| [`api/`](./api) | **the Rust api** — the backend (axum + sqlx): every `/api/*` route except the four permanent TS residents serves from it (the TS API behind the proxy was deleted with the cutover), and the scheduler arms from it on boot unless `TALARIA_SCHEDULER=off`. The record and rules: [`docs/RUST-MIGRATION.md`](./docs/RUST-MIGRATION.md) | `bun run api` (dev, :5274); `bun run api:check` |

...
| [`api/`](./api) | **the Rust api** — the TS→Rust port of the backend (axum + sqlx; the LLM gateway serves from here today, the rest of the routes batch by batch). The TS server stays the default until cutover; roadmap and rules: [`docs/RUST-MIGRATION.md`](./docs/RUST-MIGRATION.md) | `bun run api` (dev, :5274); `bun run api:check` |
...
| [`docs/RUST-MIGRATION.md`](./docs/RUST-MIGRATION.md) | The TS→Rust api port: the rules, the coexistence switch, the batches, recorded divergences |

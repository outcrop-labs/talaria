...
- **The Rust API** ✅. The backend — every `/api` route (the four permanent TS residents
  aside: `admin/update`, app dispatch, the app-MCP gateway, and `healthz`), the SSE
  streams, and the scheduler — runs on the Rust crate ([`api/`](./api)), byte-diffed
  against the TS oracle across 370 route pairs before the TS API was deleted with the
  cutover (2026-09-01). The SvelteKit server stays the SPA host and hands `/api/*` to
  Rust on loopback — permanently. The api ships as a pre-built package image, so no
  deploy box ever compiles Rust.

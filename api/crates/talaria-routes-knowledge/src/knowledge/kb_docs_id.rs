...
use talaria_api_facades::kb::okf::{generate_doc_okf, queue_doc_okf};
...
fn doc_overlay(
    doc: &talaria_api_facades::kb::KbDoc,
    eff: &Guarded,
    governs: Option<bool>,
) -> Value {
...
fn parse_put_body(obj: &serde_json::Map<String, Value>) -> Result<PutBody, String> {
    Ok(PutBody {
        ...
        icon: talaria_body::present_nullable_max_string_member(obj, "icon", 16)?,
        ...
        parent_id: talaria_body::present_nullable_uuid_member(obj, "parentId")?,
...
pub async fn put(
    ...
    if let Some(agent) = agent.clone() {
        ...
        if body.official.is_some()
            && !matches!(
                talaria_permissions::has_perm(&state.pg, &user.id, &user.role, "kb.official").await,
                Ok(true)
            )
        {
            return house_error(

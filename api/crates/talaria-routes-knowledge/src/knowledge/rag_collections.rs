...
use talaria_api_facades::retrieval::collections::{self, AccessBinding, RagCollection};
...
pub(crate) fn pg_text_to_iso(s: &str) -> String {
    ...
    chrono::DateTime::parse_from_str(s, "%Y-%m-%d %H:%M:%S%.f%#z")
        .map(|dt| talaria_agent_auth::epoch_ms_to_iso(dt.timestamp_millis()))

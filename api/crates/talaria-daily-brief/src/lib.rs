...
pub use talaria_daily_brief_delegation as delegation;
...
use talaria_harness_defs::defs::briefer::{daily_brief_lede_harness, daily_brief_note_harness};
use talaria_notify::NotifyDeps;
use talaria_realtime::{RealtimeDeps, UserEvent, publish_user};
use talaria_state::AppState;
use talaria_tz::{LocalMoment, local_moment, parse_rfc3339_ms};
...
impl From<&talaria_session::SessionUser> for BriefUser {
    ...
    fn from(user: &talaria_session::SessionUser) -> Self {
        BriefUser {
            id: user.id.clone(),
...
pub fn reader_zone(stored: Option<&str>, tz: Option<&str>, config: &BriefConfig) -> String {
    ...
    match tz.filter(|t| !t.is_empty() && t.len() <= 64) {
        Some(t) if talaria_me::is_valid_time_zone(t) => t.to_string(),
...
async fn write_lede(
    ...
    date: &str,
    zone: &str,
) -> Result<Option<String>, talaria_harness::run::HarnessError> {
...
async fn write_note(
    ...
    row: &BriefRow,
    appends: &[NewEntry],
) -> Result<Option<NewEntry>, talaria_harness::run::HarnessError> {
...
pub fn daily_brief_job_spec(deps: Arc<BriefDeps>) -> talaria_scheduler::JobSpec {
...
pub fn register_daily_brief_job(deps: Arc<BriefDeps>) {
    talaria_scheduler::register_job(daily_brief_job_spec(deps));
}
...
mod tests {
    ...
    fn badges_read_the_due_date_against_the_clock() {
        ...
        let now = talaria_agent_auth::iso_to_epoch_ms("2026-08-29T12:00:00Z").unwrap();

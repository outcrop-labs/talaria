...
use crate::provider::{http, native_base, openrouter_us_pool, resolve_endpoint_key};
use crate::registry::ResolvedRoute;
use crate::vault::{SecretVault, seal_content};
...
async fn fetch_inner(
    ...
    for _attempt in 0..4 {
        ...
        if !present || classify_param(&param) == crate::params::ParamClass::Protected {
            return Ok(Reply::Synthesized {
                status: 400,
                text,
                content_type,
        ...
        if let Some(cap) = contract_capability(&param) {
            ...
            if let Some(r) = route {
                let pg = pg.clone();
                let key = crate::params::capability_key(&r.endpoint.name, &r.upstream_model);
...
pub fn gateway_pulse() -> GatewayPulse {
    let cutoff = crate::params::now_ms() - 15 * 60_000;

...
pub async fn set_storage_config(pg: &PgPool, sb: &SecretBox, cfg: &StorageConfig) {
    ...
    let _ = talaria_gateway::settings::set_setting(pg, KEY, &value).await;
}
...
pub async fn s3_put(t: &BucketTarget, key: &str, bytes: &[u8], mime: &str) -> Result<(), String> {
    ...
    if !status.is_success() {
        ...
        return Err(format!(
            "storage PUT {}: {}",
            status.as_u16(),
            talaria_body::truncate_utf16(text.trim(), 300)
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn unseal_never_panics_and_fails_closed() {
        let sb = talaria_secretbox::SecretBox::from_parts(
            talaria_secretbox::derive_kek("test-root"),

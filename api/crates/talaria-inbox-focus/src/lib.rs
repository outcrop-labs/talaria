...
use crate::sources::{approval_items, channel_items, notification_items, task_items};
...
use talaria_channels::{channel_role, insert_channel_message, mark_channel_read};
use talaria_daily_brief::focus::key_of;
use talaria_fleet_agents::routed_model_for;
use talaria_google_pending::{PendingAction, decide_action, list_pending};
use talaria_harness::run::{
    DeltaFn, HarnessError, RunContext, StreamFn, StreamOptions, run_harness, run_harness_streamed,
};
use talaria_harness::transport::{TransportRequest, dispatch_stream};
...
use talaria_inbox_focus_types::{AssistantBrief, FocusAssistant, RawFocusItem, wire_item};
use talaria_model_access::gateway_models_for;
use talaria_notify::NotifyDeps;
use talaria_session::{SessionUser, actor_of};
use talaria_state::AppState;
use talaria_statuses::status_meta;
use talaria_tasks_types::{TaskDeps, TaskError};
...
pub static GET_TASK: OnceLock<
    ...
    >,
> = OnceLock::new();
pub static COMPLETE_QUALITY_REVIEW: OnceLock<
    Arc<
        dyn Fn(TaskDeps, String, String, String, String) -> BoxFuture<'static, TaskErrorResult>
            + Send
            + Sync,
    >,
> = OnceLock::new();
type TaskErrorResult = talaria_tasks_types::TaskResult<Option<Task>>;
...
fn finished_reply(value: Option<Value>, max: usize) -> Option<String> {
    value
        ...
        .map(|text| talaria_body::truncate_utf16(&text, max).to_string())
...
async fn proposed_confirmation(
    ...
    let expires_iso = talaria_agent_auth::epoch_ms_to_iso(now_ms() + CONFIRMATION_MS);
...
async fn consume_confirmation(
    ...
    let Some((proposal, agent_model, delegate_model)) = confirmed.filter(|(p, _, _)| p.is_object())
        ...
        let expires_iso = expires_ms.map(talaria_agent_auth::epoch_ms_to_iso);
        ...
        && expires_ms > now_ms()
    {
            ...
            && token_hash(&token) == hash
        {
            return Ok(json!({
                ...
                "decisionId": decision_id,
                "confirmationToken": token,
                "expiresAt": talaria_agent_auth::epoch_ms_to_iso(expires_ms),
                "preview": preview,
            }));
        }
    }
    ...
    match status.as_str() {
        "completed" => {
            ...
            if action_id == "mark_read" && completed_ms.unwrap_or(0) > now_ms() - UNDO_MS {
                result["undo"] = json!({
                    "decisionId": decision_id,
                    "expiresAt": talaria_agent_auth::epoch_ms_to_iso(
...
async fn execute_action_arms(
    ...
    if item.source_type == "notification" && action_id == "mark_read" {
        ...
        outcome = json!({
            "beforeReadAt": null,
            "afterReadAt": talaria_agent_auth::epoch_ms_to_iso(after),
        });
        ...
        let message = talaria_body::truncate_utf16(
            payload
                .get("message")
                .and_then(Value::as_str)
                .unwrap_or("")
    ...
    if action_id == "mark_read" {
        result["undo"] = json!({
            "decisionId": decision_id,
            "expiresAt": talaria_agent_auth::epoch_ms_to_iso(now_ms() + UNDO_MS),
        });
    }
    Ok(result)
}
...
pub async fn valid_response_model(
    pg: &PgPool,
    user: &SessionUser,
    requested: Option<&str>,

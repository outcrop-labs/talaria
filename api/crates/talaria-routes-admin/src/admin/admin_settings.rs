...
pub async fn get(State(state): State<AppState>, headers: axum::http::HeaderMap) -> Response {
    ...
    Json(serde_json::json!({
        ...
        "memberModels": talaria_api_facades::model::access::member_model_allowlist(&state.pg).await,
        ...
        Some(b) => {
            ...
            match o.get("agents") {
                ...
                Some(v) => {
                    let a = v
                        .as_object()
                        .ok_or_else(|| talaria_body::record_msg(zod_type_name(v)))?;
            ...
            let assembled = |agents: serde_json::Map<String, Value>| {
                Value::Object({
                    ...
                    m.insert(
                        "windowHours".into(),
                        talaria_body::js_num(window_hours).into(),
                    );
...
pub async fn put(
    ...
    if let Some(models) = &body.member_models {
        talaria_api_facades::model::access::set_member_model_allowlist(&state.pg, models).await;
        log_audit(
            &state.pg,
            AuditEntry {
                actor: &actor,
    ...
    if body.org_present {
        ...
        tokio::spawn(async move {
            if let Some(sb) = sb {
                let _ = talaria_api_facades::fleet::reconcile::roll_running_agents(&pg, &sb).await;
            }
        });
        log_audit(
            &state.pg,

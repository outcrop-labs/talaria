...
pub async fn get(
    ...
    let (active, counts) = match (active, counts) {
        ...
        (Err(e), _) | (_, Err(e)) => {
            return Ok(internal("[admin/encryption] status read failed", e));
        }
    };
    ...
    Ok(Json(serde_json::json!({
        ...
        "rotatedAt": active
            .map(|(_, ms)| talaria_agent_auth::epoch_ms_to_iso(ms)),
        "secretCount": counts,
...
pub async fn post(
    ...
    match &parsed {
        ...
        v => {
            ...
            if let Some(v) = o.get("newRootSecret") {
                ...
                if talaria_body::utf16_len(s) < 16 {
                    return Ok(house_error(
                        StatusCode::BAD_REQUEST,
                        "new root secret must be at least 16 chars",
                    ));
                }
                if talaria_body::utf16_len(s) > 400 {
                    return Ok(house_error(
                        StatusCode::BAD_REQUEST,
                        &talaria_body::too_big_msg(400),
                    ));
                }
    ...
    let sb = secretbox_or_500(&state, "[admin/encryption] secretbox unavailable").await?;
    Ok(

...
use talaria_body::{
    NumKind, array_msg, array_too_big_msg, array_too_small_msg, boolean_member,
...
pub async fn get(
    ...
    let agent = talaria_api_facades::google::oauth::query_pairs(uri.query())
        .get("agent")
        .filter(|a| !a.is_empty())
        .cloned();
    if let Some(agent) = agent {
        return Ok(match handles_held_by(&state.pg, &agent).await {
    ...
    let secrets = match list_secret_docs(&state.pg).await {
        Ok(s) => s,
        Err(e) => return Ok(internal("[admin/workspace-secrets] list failed", e)),
    };
    let folders = match list_secret_folders(&state.pg, &user.id, true).await {
        Ok(f) => f,
        Err(e) => return Ok(internal("[admin/workspace-secrets] folders read failed", e)),
    };
    Ok(Json(json!({ "secrets": secrets, "folders": folders })).into_response())
}
    ...
    Ok(match action {
        ...
        Some("create") => {
            ...
            if utf16_len(&name) > 40 {
                return Ok(house_error(StatusCode::BAD_REQUEST, &too_big_msg(40)));
            }
            ...
            if arr.is_empty() {
                return Ok(house_error(
                    StatusCode::BAD_REQUEST,
                    &array_too_small_msg(1),
                ));
            }
            if arr.len() > 20 {
                return Ok(house_error(StatusCode::BAD_REQUEST, &array_too_big_msg(20)));
            }
            ...
            let sb = match state.secretbox().await {
                ...
                Err(e) => {
                    return Ok(internal(
                        "[admin/workspace-secrets] secretbox unavailable",
                        e,
                    ));
                }
            };
        ...
        Some("grant") | Some("revoke") => {
            ...
            if let Err(e) = result {
                return Ok(internal("[admin/workspace-secrets] grant/revoke failed", e));
            }
            log_audit(
                &state.pg,
                AuditEntry {
        ...
        Some("delete") => {
            ...
            if let Err(e) = delete_secret_doc(&state.pg, &name).await {
                return Ok(internal("[admin/workspace-secrets] delete failed", e));
            }
            log_audit(
                &state.pg,
                AuditEntry {
        ...
        Some("folder-create") => {
            ...
            let f = match create_secret_folder(&state.pg, &name, None).await {
                ...
                Err(e) => {
                    return Ok(internal(
                        "[admin/workspace-secrets] folder create failed",
                        e,
                    ));
                }
            };
        ...
        Some("folder-delete") => {
            ...
            let deleted = match delete_secret_folder(&state.pg, &id, &user.id, true).await {
                ...
                Err(e) => {
                    return Ok(internal(
                        "[admin/workspace-secrets] folder delete failed",
                        e,
                    ));
                }
            };
            if !deleted {
                return Ok(house_error(StatusCode::NOT_FOUND, "no such folder"));
            }
            log_audit(
                &state.pg,
                AuditEntry {
        ...
        Some("folder-grant") => {
            ...
            {
                Ok(s) => s,
                Err(e) => return Ok(internal("[admin/workspace-secrets] folder grant failed", e)),
            };
            if !shared {
                return Ok(house_error(StatusCode::NOT_FOUND, "no such folder"));
            }
        ...
        Some("file") => {
            ...
            let folder_id = match present_nullable_uuid_member(obj, "folderId") {
                Ok(Some(f)) => f,
                Ok(None) => return Ok(house_error(StatusCode::BAD_REQUEST, "Invalid input")),
                Err(msg) => return Ok(house_error(StatusCode::BAD_REQUEST, &msg)),
            };
            let moved =
                ...
                {
                    Ok(m) => m,
                    Err(e) => return Ok(internal("[admin/workspace-secrets] file failed", e)),
                };
            if !moved {
                return Ok(house_error(StatusCode::BAD_REQUEST, "could not file that"));
            }
            log_audit(
                &state.pg,
                AuditEntry {

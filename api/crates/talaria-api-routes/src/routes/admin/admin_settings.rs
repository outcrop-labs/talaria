...
pub async fn get(State(state): State<AppState>, headers: axum::http::HeaderMap) -> Response {
    ...
    Json(serde_json::json!({
        ...
        "memberModels": crate::model::access::member_model_allowlist(&state.pg).await,
...
pub async fn put(
    ...
    if let Some(models) = &body.member_models {
        crate::model::access::set_member_model_allowlist(&state.pg, models).await;
        log_audit(
            &state.pg,
            AuditEntry {
                actor: &actor,
    ...
    if body.org_present {
        ...
        tokio::spawn(async move {
            if let Some(sb) = sb {
                let _ = crate::fleet::reconcile::roll_running_agents(&pg, &sb).await;
            }
        });
        log_audit(
            &state.pg,

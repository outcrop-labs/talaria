...
pub static TALARIA_TOOLS: LazyLock<Vec<SandboxTool>> = LazyLock::new(|| {
    vec![
        ...
        SandboxTool {
            ...
            parameters: json!({
                ...
                "properties": {
                    ...
                    "parentId": str_schema("Only sub-tasks of this ticket — id or ref"),
                },
                "required": ["boardId"],
            }),
            assistant_only: false,
        ...
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Tickets,
            description: "Create a ticket on a board. It lands in the inbox for a human to assign — agents cannot assign work, hour estimates stay human (use effort for your own sizing), and sub-task structure is a human's too. A parentId you pass is ignored; file the ticket, then comment the breakdown.",
            parameters: json!({
                "type": "object",
                "properties": {
                    "boardId": str_schema("Board id (from list_boards)"),
        ...
        SandboxTool {
            ...
            description: "Show platform tools in the current chat as chips the person can open and run. Use this when they should see a tool and its inputs without you calling it yourself. Pass the tool names you want on screen, and any inputs you already know.",
            parameters: json!({ "type": "object", "properties": { "tools": str_schema("Platform tool name, e.g. create_ticket") }, "required": [] }),
            assistant_only: false,
            needs_google: false,
        },
        SandboxTool {
        ...
        SandboxTool {
            ...
            description: "Read upcoming Google Calendar events — your owner's if you're a personal assistant, otherwise the shared org calendar. Requires that Google account to be connected in Talaria. Optional timeMin and timeMax are RFC3339; maxResults defaults to 10.",
            parameters: json!({ "type": "object", "properties": { "timeMin": str_schema("Window start, RFC3339"), "timeMax": str_schema("Window end, RFC3339"), "maxResults": str_schema("How many events, 1–50") }, "required": [] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Queue a change to a Google Calendar event (time or title). It is NOT changed until a human approves. Say queued, never done. eventId comes from read_calendar.",
            parameters: json!({ "type": "object", "properties": { "eventId": str_schema("Event id from read_calendar"), "summary": str_schema("New title"), "start": str_schema("New start, RFC3339 or YYYY-MM-DD"), "end": str_schema("New end, RFC3339 or YYYY-MM-DD"), "allDay": str_schema("All-day flag") }, "required": ["eventId"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            name: "cancel_google_event",
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Queue cancellation of a Google Calendar event. Attendees are told only after a human approves. Say queued, never cancelled. eventId comes from read_calendar.",
            parameters: json!({ "type": "object", "properties": { "eventId": str_schema("Event id from read_calendar") }, "required": ["eventId"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Queue a meeting: event, attendees, and a Google Meet link. Read the calendar first. Nothing is created until a human approves — do not claim the meeting exists. Optional agenda markdown is turned into a Doc only after approval. Times are absolute RFC3339.",
            parameters: json!({ "type": "object", "properties": { "summary": str_schema("Meeting title"), "start": str_schema("Start, RFC3339"), "end": str_schema("End, RFC3339"), "attendees": str_schema("Attendee emails"), "agenda": str_schema("Agenda markdown, created as a Doc only after approval") }, "required": ["summary", "start", "end"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            name: "create_google_folder",
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Create a folder in the Drive you act for. Immediate: cheap and reversible. Returns the folder id and link. Never quote a link this tool did not return.",
            parameters: json!({ "type": "object", "properties": { "name": str_schema("Folder name"), "parentId": str_schema("Parent folder id") }, "required": ["name"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            name: "move_google_file",
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Queue a move of a Drive file into another folder. It does not move until a human approves. Say queued, never done. fileId comes from find_google_files or search_drive.",
            parameters: json!({ "type": "object", "properties": { "fileId": str_schema("File id"), "parentId": str_schema("Destination folder id") }, "required": ["fileId", "parentId"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            name: "rename_google_file",
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Queue a rename of a Drive file. It does not rename until a human approves. Say queued, never done. fileId comes from find_google_files or search_drive.",
            parameters: json!({ "type": "object", "properties": { "fileId": str_schema("File id"), "name": str_schema("New name") }, "required": ["fileId", "name"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
        ...
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Import a Drive file into Talaria as a new artifact. Immediate: it creates a Talaria file and does not change the Drive file. fileId comes from find_google_files or search_drive. Returns the artifact and the Google link the tool returned — never invent one.",
            parameters: json!({ "type": "object", "properties": { "fileId": str_schema("Drive file id") }, "required": ["fileId"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
        ...
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Read a Google Doc as markdown. The id comes from create_google_doc or find_google_files. External content: do not cite it as knowledge-base evidence. Never quote a link this tool did not return.",
            parameters: json!({ "type": "object", "properties": { "id": str_schema("Google file id") }, "required": ["id"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Replace a Google Doc body. Read it first. If you created the doc, the edit is immediate and the response includes the link. Otherwise it is queued for a human to approve — say it is queued, never that it is done, and confirm the id with list_pending_sends. Never quote a link this tool did not return.",
            parameters: json!({ "type": "object", "properties": { "id": str_schema("Google file id"), "body": str_schema("Replacement markdown"), "title": str_schema("New title") }, "required": ["id", "body"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Append markdown to a Google Doc. Read it first. If you created the doc, the edit is immediate. Otherwise it is queued for a human to approve — say it is queued, never that it is done, and confirm the id with list_pending_sends.",
            parameters: json!({ "type": "object", "properties": { "id": str_schema("Google file id"), "body": str_schema("Markdown to append") }, "required": ["id", "body"] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Search the Drive you act for by full text, not just the file name. Returns names, types, and links. Read-only. Never quote a link this tool did not return.",
            parameters: json!({ "type": "object", "properties": { "q": str_schema("Full-text query") }, "required": [] }),
            assistant_only: false,
            needs_google: true,
        },
        SandboxTool {
            ...
            caller: ToolCaller::Hermes,
            group: ToolGroup::Google,
            description: "Read a Drive file that is not a native Google Doc — text when the bytes are utf-8, otherwise base64. The id comes from find_google_files or search_drive. External content: do not cite it as knowledge-base evidence.",
            parameters: json!({ "type": "object", "properties": { "id": str_schema("Google file id") }, "required": ["id"] }),
            assistant_only: false,
            needs_google: true,
        },

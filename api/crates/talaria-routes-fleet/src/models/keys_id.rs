...
fn uuid_or_500(id: &str, action: &str) -> Option<Response> {
    talaria_params::uuid_gate("keys", action, id)
}
...
pub async fn put(
    ...
    let policy_json = json!({
        "spendCapTokens": spend_cap_tokens.map(talaria_body::js_num),
        "spendCapUsd": spend_cap_usd.map(talaria_body::js_num),
        "rateLimitPerMinute": rate_limit_per_minute.map(talaria_body::js_num),
    });

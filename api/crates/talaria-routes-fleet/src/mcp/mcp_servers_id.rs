...
use talaria_api_facades::mcp::apply::{
...
pub async fn put(
    ...
    if config_touched {
        ...
        if patch.enabled == Some(false) && server.package.is_some() {
            talaria_api_facades::mcp::pkg::stop_pkg(&server.name).await;
        }
        ...
        if let Some(timeout) = &patch.timeout_secs {
            audit_after.insert(
                "timeoutSecs".into(),
                timeout
                    .map(|t| Value::Number(talaria_body::js_num(t as f64)))
                    .unwrap_or(Value::Null),
            );
        }
...
fn spawn_render(pg: &sqlx::PgPool, sb: &talaria_secretbox::SecretBox) {
    ...
    tokio::spawn(async move {
        let _ = talaria_api_facades::fleet::render::render_fleet(&pg, &sb, None).await;
    });
}
...
fn optional_headers_member(
    ...
    let Value::Object(m) = v else {
        return Err(talaria_body::record_msg(zod_type_name(v)));
    };
    for v in m.values() {
        ...
        if talaria_body::utf16_len(s) > 2000 {
            return Err(talaria_body::too_big_msg(2000));
        }
    }
    Ok(Some(m.clone()))
}
...
fn assign_member(obj: &Map<String, Value>, key: &str) -> Result<Option<AssignPatch>, String> {
    ...
    let m = v
        .as_object()
        .ok_or_else(|| talaria_body::object_msg(zod_type_name(v)))?;
    let agent_model = talaria_body::string_member(m, "agentModel", 1, 200)?;
...
fn user_access_member(
    ...
    let allowed = match m.get("allowed") {
        None => Err(talaria_body::boolean_msg("undefined")),
        Some(Value::Null) => Ok(None),
        Some(_) => talaria_body::boolean_member(m, "allowed").map(Some),
    }?;
    let tools = match m.get("tools") {
        None => Err(array_msg("undefined")),
        Some(Value::Null) => Ok(None),
...
fn oauth_client_member(
    ...
    let m = v
        .as_object()
        .ok_or_else(|| talaria_body::object_msg(zod_type_name(v)))?;
    let client_id = talaria_body::string_member(m, "clientId", 1, 200)?;
    let client_secret = talaria_body::nullable_optional_string_member(m, "clientSecret", 500)?;
...
fn parse_patch(obj: &Map<String, Value>) -> Result<Patch, String> {
    ...
    let timeout_secs = match obj.get("timeoutSecs") {
        ...
        Some(_) => {
            let n = talaria_body::nullable_number_member(
                obj,
                "timeoutSecs",
                talaria_body::NumKind::Int,
                f64::MIN,
                f64::INFINITY,
            )?
...
fn optional_string_member_max(
    ...
    match obj.get(key) {
        None => Ok(None),
        Some(_) => talaria_body::string_member(obj, key, 1, max).map(Some),
    }
}

...
use sqlx::PgPool;
use talaria_api_facades::mcp::oauth::{ensure_oauth_config, has_oauth_tokens, oauth_meta};
        ...
        Some(Value::Array(a)) => {
            ...
            for arg in a {
                ...
                if m.get("type").and_then(Value::as_str) == Some("named") {
                    ...
                    if !talaria_api_facades::mcp::pkg::ALLOWED_FLAGS.contains(&flag) {
                        return Err(format!(
...
pub async fn get(State(state): State<AppState>, headers: HeaderMap) -> Result<Response, Response> {
    ...
    for s in &servers {
        ...
        if let Some(o) = wire.as_object_mut() {
            ...
            let pkg_status = if let Some(spec) = s
                .package
                .as_ref()
                .and_then(talaria_api_facades::mcp::pkg::PkgSpec::of)
            {
                json!(talaria_api_facades::mcp::pkg::pkg_status(&spec, &s.name).await)
            } else {
                Value::Null
            };
    ...
    let outcome = create_and_sniff(
        ...
        &NewServer {
            ...
            env_enc: match &package {
                Some(_) => {
                    let sealed = talaria_api_facades::mcp::pkg::SealedDoc::seal(
                        &sb,
    ...
    if let Some(doc) = &server.package {
        ...
        if !image.is_empty() {
            tokio::spawn(async move {
                talaria_api_facades::mcp::pkg::pull_and_pin(&pg, &id, &image).await;
            });
        }
    }
...
async fn create_and_sniff(
    pg: &PgPool,
    input: &NewServer<'_>,
) -> Result<talaria_api_facades::mcp::registry::McpServer, String> {
...
fn spawn_audit_and_render(
    ...
    server: &talaria_api_facades::mcp::registry::McpServer,
) {
    let pg = pg.clone();
    let sb = sb.clone();

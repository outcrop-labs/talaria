...
use crate::fleet::fleet_endpoints::price_record;
use axum::Json;
use axum::extract::{Path, State};
use axum::http::{HeaderMap, StatusCode};
use axum::response::{IntoResponse, Response};
use serde_json::{Value, json};
use talaria_api_facades::fleet::cascade::{ModelUsage, cascade_removal, model_usage};
use talaria_api_facades::gateway::registry::{
    EndpointPatch, delete_endpoint, list_endpoints, update_endpoint,
};
...
fn validate_patch(obj: &serde_json::Map<String, Value>) -> Result<ValidatedPatch, String> {
    ...
    let request_defaults = match obj.get("requestDefaults") {
        ...
        Some(v) => {
            return Err(talaria_body::record_msg(talaria_body::zod_type_name(v)));
        }
    };
...
fn nullish_nonneg(
    ...
    match obj.get(key) {
        ...
        Some(v @ Value::Number(_)) => {
            ...
            if f < 0.0 {
                return Err(talaria_body::too_small_msg(0));
            }
            Ok(Some(Some(f)))
        }
        Some(v) => Err(format!(
...
fn effort_record(v: &Value, key_max: usize) -> Result<Value, String> {
    let map = v
        .as_object()
        .ok_or_else(|| talaria_body::record_msg("object"))?;
    let mut out = serde_json::Map::new();
    for (k, val) in map {
        if talaria_body::utf16_len(k) > key_max {
            return Err(talaria_body::too_big_msg(key_max));
        }
        let arr = val.as_array().ok_or_else(|| {
            format!(
                "Invalid input: expected array, received {}",
                talaria_body::zod_type_name(val)
            )
        })?;
        if arr.is_empty() {
            return Err(talaria_body::array_too_small_msg(1));
        }
        if arr.len() > 12 {
            return Err(talaria_body::array_too_big_msg(12));
        }
        let mut ladder = Vec::with_capacity(arr.len());
        for el in arr {
            let s = el
                .as_str()
                .ok_or_else(|| talaria_body::string_msg(talaria_body::zod_type_name(el)))?;
            let n = talaria_body::utf16_len(s);
            if n < 1 {
                return Err(talaria_body::too_small_msg(1));
            }
            if n > 24 {
                return Err(talaria_body::too_big_msg(24));
            }

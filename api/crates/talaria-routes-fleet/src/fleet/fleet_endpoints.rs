...
use talaria_api_facades::gateway::provider::migrate_env_keys_to_cipher;
...
fn validate(obj: &serde_json::Map<String, Value>) -> Result<Validated, String> {
    ...
    let base_url = match obj.get("baseUrl") {
        ...
        Some(_) => Some(talaria_body::url_member(obj, "baseUrl", 300)?),
    };
    let class = talaria_body::enum_member(obj, "class", &["local", "cloud"])?;
...
fn nullish_key_env(
    ...
    match obj.get(key) {
        ...
        Some(v) => {
            let s = v
                .as_str()
                .ok_or_else(|| talaria_body::string_msg(talaria_body::zod_type_name(v)))?;
            if !talaria_api_facades::gateway::provider::key_env_allowed(s) {
                return Err(
                    "Invalid string: must match pattern /^(LLM_API_KEY|[A-Z][A-Z0-9_]*_API_KEY)$/"
                        .to_string(),
                );
            }
            if talaria_body::utf16_len(s) > 80 {
                return Err(talaria_body::too_big_msg(80));
            }
            Ok(Some(s.to_string()))
        }
    }
            ...
            match entry.get(field) {
                ...
                Some(other) => {
                    return Err(format!(
                        ...
                        talaria_body::zod_type_name(other)
                    ));
                }
            }
        }

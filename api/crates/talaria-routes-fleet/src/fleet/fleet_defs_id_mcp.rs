...
use talaria_agent_defs::{add_version_if_changed, list_versions};
use talaria_agent_mcp::apply_mcp_edits;
...
fn parse_add(obj: &Map<String, Value>) -> Result<Vec<AddServer>, String> {
    ...
    for el in arr {
        let m = el
            .as_object()
            .ok_or_else(|| talaria_body::object_msg(zod_type_name(el)))?;
        ...
        if talaria_body::utf16_len(&name) > 60 {
        ...
        let timeout = match m.get("timeout") {
            ...
            Some(_) => {
                ...
                Some(Value::from(talaria_body::js_num(n)))
            }
        };
...
fn parse_remove(obj: &Map<String, Value>) -> Result<Vec<String>, String> {
    ...
    for el in arr {
        ...
        if talaria_body::utf16_len(s) > 60 {
            return Err(too_big_msg(60));
        }

...
pub async fn get(
    ...
    let agents: Vec<serde_json::Value> = {
        ...
        keys.into_iter()
            .map(|k| {
                let leak = talaria_api_facades::fleet::resources::mem_is_leaking(
    ...
    let host = talaria_api_facades::fleet::budget::host_mem().await.map(|h| {
        let reserve = talaria_api_facades::fleet::budget::host_reserve_for(h.total);
        json!({
            "total": h.total,
            "available": h.available,
            "reserve": reserve,
            "agentCeiling": talaria_api_facades::fleet::budget::workbench_limit_for(Some(&h)),

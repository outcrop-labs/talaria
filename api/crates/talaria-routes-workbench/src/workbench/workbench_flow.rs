...
use talaria_body::{nullish_max_string_member, parse, string_member};
...
pub async fn get(State(state): State<AppState>, headers: HeaderMap) -> Result<Response, Response> {
    ...
    let flows = match gh::list_repo_flows(&state.pg).await {
        Ok(f) => f,
        Err(e) => return Ok(internal("[workbench/flow] flow read failed", e)),
    };
    let repos = gh::list_reachable_repos(&state.pg, &sb).await;
    Ok(Json(json!({ "flows": flows, "repos": repos })).into_response())
}
...
pub async fn put(
    ...
    Ok(Json(json!({ "flows": flows })).into_response())
}

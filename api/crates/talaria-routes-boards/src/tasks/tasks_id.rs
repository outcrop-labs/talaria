...
use talaria_workflows::MatchTarget;
...
fn refs_member(obj: &serde_json::Map<String, Value>) -> Result<Option<Vec<MessageRef>>, String> {
    match obj.get("refs") {
        ...
        Some(Value::Array(items)) => {
            if items.len() > 3 {
                return Err(talaria_body::array_too_big_msg(3));
            }
            ...
            for item in items {
                let Some(entry) = item.as_object() else {
                    return Err(format!(
                        ...
                        talaria_body::zod_type_name(item)
                    ));
                };
                let ref_type = talaria_body::enum_member(entry, "type", &["kb-doc", "artifact"])?;
                let id = talaria_body::uuid_member(entry, "id")?;
...
pub async fn put(
        ...
        && description_input != Some(task.description.clone())
    {
        ...
        let (sender_id, sender_label) = match session_user.as_ref() {
            ...
            None => (
                String::new(),
                talaria_api_facades::fleet::describe_agent(&actor.id).label,
            ),
        };
...
pub async fn delete(
    ...
    // cascade on task_id) and every comment's activity point out of the
    // brain too, so no dead text answers searches while a reindex is nowhere
    // in sight. The cutover carried the old conversation messages' ids
    // verbatim into the rooms, so this sweep retires those points too.
    let comment_ids: Vec<String> = sqlx::query_scalar(
        "select m.id::text from channel_messages m \
         join channels ch on ch.id = m.channel_id \
         where ch.task_id = $1::uuid",
    )
    .bind(&id)
    .fetch_all(&state.pg)
    .await
    ...
    match delete_task(&deps, &id).await {
        Ok(()) => {
            ...
            tokio::spawn(async move {
                let qd = talaria_api_facades::retrieval::qdrant::real_deps();
                let ed = talaria_api_facades::retrieval::embed::real_deps();
                let _ = talaria_api_facades::retrieval::sources::unindex_activity(
                    &pg, &qd, &ed, "ticket", &ticket_id,
                )
                .await;
                for cid in comment_ids {
                    let _ = talaria_api_facades::retrieval::sources::unindex_activity(
                        &pg, &qd, &ed, "comment", &cid,
                    )
                    .await;
                }
            });

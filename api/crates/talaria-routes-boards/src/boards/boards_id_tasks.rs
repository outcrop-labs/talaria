...
use talaria_tasks::{NewTask, create_task, list_board_tasks};
use talaria_templates::{ResolveContext, resolve_template};
...
pub async fn post(
    ...
    if raw_description.as_deref().is_some_and(|d| d.contains('@')) {
        ...
        let (sender_id, sender_label) = if is_agent {
            (
                String::new(),
                talaria_api_facades::fleet::describe_agent(&actor).label,
            )
        } else {

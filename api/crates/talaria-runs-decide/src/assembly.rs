...
use talaria_approvals::{ApprovalDeps, announce_approval, audience_for, run_decision_approval};
use talaria_notify::{NotifyDeps, mark_brief_stale};
use talaria_realtime::{self, RealtimeDeps};
use talaria_runs_define::{RunRow, run_definition};
use talaria_runs_run::{DefinitionForFn, NowFn, PublishFn, RedisRunLease, RunDeps};
use talaria_runs_store::PgRunStore;
...
fn real_edges(pg: sqlx::PgPool, rt: RealtimeDeps) -> RealEdges {
    ...
    let publish: PublishFn = talaria_realtime_watch::run_publish(rt.clone());

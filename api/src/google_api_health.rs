...
use crate::google_connections::TokenError;
use crate::google_org::get_org_access_token;
...
pub async fn probe_org_google_apis(
    ...
    sb: &SecretBox,
    now_ms: i64,
) -> Result<Vec<GoogleApiHealth>, crate::google_errors::GoogleError> {
    let token = get_org_access_token(pg, sb, now_ms)
        .await
        .map_err(|e: TokenError| crate::google_errors::GoogleError::Failed(e.to_string()))?;

...
use crate::workchains::{Workchain, list_workchains};
use axum::Json;
...
pub async fn get(
    ...
    if let Some(gate) = crate::params::uuid_gate("boards", "GET workchains", &id) {
        return gate;
    }
...
pub async fn post(
    ...
    if let Some(gate) = crate::params::uuid_gate("boards", "POST workchains", &id) {
        return gate;
    }
    match board_role(&state.pg, &user.id, &id).await {

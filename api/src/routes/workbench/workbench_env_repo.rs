...
fn max_string_member_guard(
    ...
    match obj.get(key) {
        Some(serde_json::Value::String(s)) => {
            let n = s.chars().count();
            if n < min {
                Err(crate::body::too_small_msg(min))
            } else if n > max {
                Err(crate::body::too_big_msg(max))
            } else {
                Ok(s.clone())
            }
        }
        Some(v) => Err(crate::body::string_msg(crate::body::zod_type_name(v))),
        None => Err(crate::body::string_msg("undefined")),
    }
}

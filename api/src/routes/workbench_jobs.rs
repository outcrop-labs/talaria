...
use crate::workbench_mcp::{MergeJob, WorkbenchActor, WorkbenchDeps, merge_job_to_testing};
...
pub async fn put(State(state): State<AppState>, headers: HeaderMap, body: Bytes) -> Response {
    ...
    if action == "merge_testing" {
        ...
        return match r {
            ...
            Err(e) => {
                let msg = match e {
                    crate::workbench_mcp::MergeJobError::Fail(m) => m,
                    crate::workbench_mcp::MergeJobError::Throw(m) => m,
                };

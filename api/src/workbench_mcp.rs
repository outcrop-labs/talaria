...
use crate::mcp_jsonrpc::{ListedTool, ToolOutcome, dispatch_jsonrpc};
...
async fn call_tool(
    ...
    match name {
        "doctor" => {
            ...
            if let Some(h) = h {
                checks.push(match &h.def.auth {
                    crate::workbench_harnesses::HarnessAuth::Gateway => {
                        "auth: Talaria gateway (no key needed on your side)".into()
                    }
                    crate::workbench_harnesses::HarnessAuth::Provider { provider, env_var } => {
        ...
        "start_job" => {
            ...
            let mut found: Vec<&crate::workbench_harnesses::ResolvedHarness> = profile
                .harnesses
                .iter()

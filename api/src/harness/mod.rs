...
// probes of zod 4.3.6 itself), and `prompt_rules` (the trust-boundary
// clause more than one harness states). Still TS: `define`, `transport`,
// `run`, `registry`, `gateway-params`, `recorded`, `model` (the persona
// engine's harness-facing half crossed earlier as `harness_model.rs`), and
// every def under harness/defs/.
